...
export function registerChatParticipant(context: vscode.ExtensionContext, vendor: string): void {
  const participant = vscode.chat.createChatParticipant('vidia.assistant', async (request, chatContext, response, token) => {
    ...
    if (request.command === 'explain' || request.command === 'fix') {
      ...
      if (selection)
        prompt = `${request.command === 'explain' ? 'Explain this code:' : 'Suggest a fix for this code:'}\n\n${selection}`

    }
    if (chatContext.history.length > 0)
      prompt = `Conversation so far:\n${chatContext.history.slice(-6).map(h => h instanceof vscode.ChatRequestTurn ? `User: ${h.prompt}` : h instanceof vscode.ChatResponseTurn ? 'Assistant: (see above)' : '').join('\n')}\n\n${prompt}`
